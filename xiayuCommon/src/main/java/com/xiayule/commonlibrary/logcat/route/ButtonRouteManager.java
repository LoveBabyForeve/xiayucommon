 ...
 * @Version: 1.0
 */
public class ButtonRouteManager {
    public static final String TAG = "ButtonRouteManager";

    private static ButtonRouteManager mButtonRouteManager;

    // 实例唯一化
    public static synchronized ButtonRouteManager getInstance() {
        if (mButtonRouteManager == null) {
            mButtonRouteManager = new ButtonRouteManager();
        }
        return mButtonRouteManager;
    }
     ...
     * 保存 Activity路由 到文件
     */
    public void saveButtonRoute(String str) {
        if (mIsRunning) {
            new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    if (!SPUtils.getInstance().contains(TAG)) {
                        fileName = FileUtils.getLogcatCacheFile(SavaLogcatManager.BUTTON_PATH, SavaLogcatManager.BUTTON_PATH_NAME, SavaLogcatManager.TYPE_JSON).getAbsolutePath();
